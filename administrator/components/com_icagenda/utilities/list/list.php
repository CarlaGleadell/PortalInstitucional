	...
	public static function header($total, $number_per_page, $search = false)
	{
		...
		if ($search)
		{
			...
			if ( ! $params->get('search_filters', '0') && $filter_category)
			{
				...
				$resetBtn     = '<a class="btn btn-info btn-sm ic-button-reset-category" href="' . JRoute::_('index.php?option=com_icagenda&view=events&Itemid=' . $jinput->get('Itemid')) . '">
									' . $cat_title . '&nbsp;&nbsp;&nbsp;<span class="icon-cancel icon-white"></span>
								</a>';

				$header_title = JText::_('COM_ICAGENDA_HEADER_SEARCH_TITLE') . '<br />' . $resetBtn;
				$header_sub   = JText::plural('COM_ICAGENDA_HEADER_SEARCH_FILTER_CATEGORY_N_EVENTS', $total);
