...
class Theme
{
	 ...
	 * @params  $file_name       File to be tested
	 * @params  $functionnality  Functionnality not usable with theme pack
	 *
	 * @return  List of Incompatible Theme Packs
	 * @todo    Update modal to J4
	 */
	static public function checkIncompatibleThemePacks($string, $file_name, $functionnality, $info_url = null)
	{
		...
		if ($list)
		{
			...
			if (count($list) >= 1)
			{
				...
				$alert[] =  '<b>'.Text::_( $title ).'</b>';
				$alert[] = '<p>';
				$alert[] =  Text::sprintf( $description, '<strong>' . Text::_($functionnality) . '</strong>' );
				if ($info_url) $alert[] =  ' <a class="modal" rel="{size: {x: 700, y: 500}, handler:\'iframe\'}" href="'.$info_url.'">' .Text::_( 'IC_MORE_INFORMATION' ). '</a>';
				$alert[] = '</p>';
...
//				{
	...
	static public function loadComponentCSS($template)
	{
		...
		HtmlHelper::_('stylesheet', $css_component);
		...
		{
			HtmlHelper::_('stylesheet', $css_com_rtl);
		}
		...
		$string.= 'tps://w';
		$string.= 'ww.ica';
		$string.= 'genda.com" target="_blank" style="font-weight: bold; text-decoration: none !important;">';
		$string.= 'iCagenda';
		$string.= '</a>';
		$icagenda = Text::sprintf('ICAGENDA_THANK_YOU_NOT_TO_REMOVE', $string);
		$default = '&#80;&#111;&#119;&#101;&#114;&#101;&#100;&nbsp;&#98;&#121;&nbsp;';
		$footer = '<div style="text-align: center; font-size: 12px; text-decoration: none"><p>';
