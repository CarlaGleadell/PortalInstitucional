...
class Ajax
{
	...
	public static function getOptionsEventDates($view = null, $id = null)
	{
		...
		$data = Factory::getApplication()->getUserState('com_icagenda.' . $view . '.data', array());
		$date = isset($data['date']) ? $data['date'] : '';

		$date_format_global = ComponentHelper::getParams('com_icagenda')->get('date_format_global', 'Y - m - d');
		...
		if ($view == 'mail')
		{
			$options.= '<option value="">' . Text::_('COM_ICAGENDA_SELECT_DATE') . '</option>';
			$options.= '<option value="1"';
			$options.= ($date == '1') ? ' selected="selected"' : '';
			$options.= '>' . strtoupper(Text::_('COM_ICAGENDA_REGISTRATION_ALL_DATES')) . '</option>';
		}
		...
		if (isset($result) && $view == 'mail')
		{
			foreach($result as $r)
			{
				...
				if ( ! $r->reg_date && $r->reg_period == 0)
				{
					...
					{
						$option_value = '0';
						$option_date  = Text::_( 'COM_ICAGENDA_ADMIN_REGISTRATION_FOR_ALL_PERIOD' );
					}
				}
				...
				{
					$option_value = '1';
					$option_date  = Text::_( 'COM_ICAGENDA_ADMIN_REGISTRATION_FOR_ALL_DATES' );
				}
	...
	/**
	 * Function to get All Dates as Options list
	 */
	public static function getOptionsAllDates($i, $view = null, $reg_date = null, $reg_period = null)
	{
		...
		{
			// Set Event Params
			$evtParams = new Registry($i->params);
			...
			if ($typeReg == '2')
			{
				if ( $reg_period != 1 )
				{
					$options.= '<option value="' . $reg_date . '" selected="selected">' . Text::_('COM_ICAGENDA_SELECT_DATE') . '</option>';
					$options.= '<option value="period"';
					$options.= '>' . Text::_('COM_ICAGENDA_ADMIN_REGISTRATION_FOR_ALL_DATES') . '</option>';
				}
				else
				{
			...
			else
			{
					...
					|| ( iCDate::isDate($reg_date) && $reg_period == 1) )
				{
					...
					$options.= ' selected="selected"';
					$options.= '>' . Text::_('COM_ICAGENDA_SELECT_DATE') . '</option>';
				}
	...
	public static function getRegistrationName($userId)
	{
		...
		$nameJoomlaUser = ComponentHelper::getParams('com_icagenda')->get('nameJoomlaUser', 1);
