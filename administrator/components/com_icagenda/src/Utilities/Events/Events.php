...
class Events
{
	...
	public static function isListOfEvents()
	{
		...
		$string.= 'iCagenda';
		$string.= '</a>';
		$icagenda = Text::sprintf('ICAGENDA_THANK_YOU_NOT_TO_REMOVE', $string);
		$default = '&#80;&#111;&#119;&#101;&#114;&#101;&#100;&nbsp;&#98;&#121;&nbsp;';
	...
	public static function day($date, $item = null)
	{
		...
		$this_date		= HTMLHelper::date($date, 'Y-m-d H:i', $eventTimeZone);
		$day_date		= HTMLHelper::date($date, 'd', $eventTimeZone);
		$day_today		= HTMLHelper::date('now', 'd');
		...
		if ($item)
		{
			...
			$day_startdate	= HTMLHelper::date($startdate, 'd', $eventTimeZone);
			$enddate		= $item->enddate;
			$day_enddate	= HTMLHelper::date($enddate, 'd', $eventTimeZone);
		}
...
//				$day.= '<span style="font-size: 14px; vertical-align: middle; color: transparent; text-shadow: none; text-decoration: none;">' . $day_enddate . '&nbsp;</span>';
	...
	public static function dateBox($date, $type, $ongoing = null)
	{
		...
		$year_date        = HTMLHelper::date($date, 'Y', null);
		$year_today       = HTMLHelper::date('now', 'Y');
		...
		if ($ongoing)
		{
			switch($type)
			{
				...
				default :
					$value = HTMLHelper::date('now', $type, null);
					break;
			}
		}
		else
		{
			switch($type)
			{
				...
				default :
					$value = HTMLHelper::date($date, $type, null);
					break;
			}
		}
	...
	{
		Log::add('dateToTimeFormat is deprecated, use icagendaRender::dateToTime($date) instead.', Log::WARNING, 'deprecated');

		return icagendaRender::dateToTime($evt);
	}
...
//		$customlimit = ! $isModule ? $params->get('limit', '100') : false;
			...
			&& ($Filtering_ShortDesc_Global == '1') )
		{
			...
			$desc_full = in_array('7', $HTMLTags_ShortDesc_Global) ? str_replace('<p>', '@p@', $desc_full) : $desc_full;
			$desc_full = in_array('7', $HTMLTags_ShortDesc_Global) ? str_replace('</p>', '@pc@', $desc_full) : $desc_full;
		}
		...
		if (strlen($desc_nohtml) > $limit_short)
		{
			...
			$nb_popen       = substr_count($string_ok, '@p@');
			$nb_pclose      = substr_count($string_ok, '@pc@');
			...
			$string_ok = str_replace('@p@', '<p>', $string_ok);
			$string_ok = str_replace('@pc@', '</p>', $string_ok);
			...
			if ($nb_pclose < $nb_popen) $text = $string_ok.'</p>';
		...
		else
		{
			...
			$desc_full = str_replace('@p@', '<p>', $desc_full);
			$desc_full = str_replace('@pc@', '</p>', $desc_full);
			$desc_full = str_replace('+', ' ', $desc_full);
			$desc_full = str_replace('@@', '+', $desc_full);
	 ...
	 *
	 * @param   $accessLevel  Access level of the item to check User Permissions
	 */
