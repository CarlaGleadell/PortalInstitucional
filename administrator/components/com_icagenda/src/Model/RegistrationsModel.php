...
/**
 * iCagenda Component Registrations Model
 */
class RegistrationsModel extends ListModel
{
	...
	protected function populateState($ordering = 'a.id', $direction = 'desc')
	{
		...
		// Filter (dropdown) Registration State
		$registration_state = $this->getUserStateFromRequest($this->context . '.filter.registration_state', 'filter_registration_state', '', 'string');
		$this->setState('filter.registration_state', $registration_state);
	...
	protected function getListQuery()
	{
		...
		// Join over the users actions (consents). @note: GROUP_CONCAT isn't supported by non-MySQL DBs.
//		$query->select('GROUP_CONCAT(uc.user_action) AS user_action')
		$query->select('GROUP_CONCAT(uc.user_action) AS user_action, GROUP_CONCAT(uc.action_subject) AS action_subject, GROUP_CONCAT(uc.action_body) AS action_body')
...
//			$query->where('(a.state = 1)');
		...
		$registration_state = $this->getState('filter.registration_state');

		if (is_numeric($registration_state))
		{
			$query->where('a.status = ' . (int) $registration_state);
		}

		// Filter by edit access
		$user = Factory::getUser();

		if ( ! $user->authorise('core.edit', 'com_icagenda')
			&& $user->authorise('core.edit.own', 'com_icagenda'))
		{
			$userId = $user->get('id');

			$query->where($db->quoteName('a.userid') . ' = :userIdReg'
					. ' OR ' . $db->quoteName('e.created_by') . ' = :userIdEvent')
				->bind(':userIdReg', $userId, ParameterType::INTEGER)
				->bind(':userIdEvent', $userId, ParameterType::INTEGER);
		}
	...
	/**
	 * Gets a list of options for Registration State Filter.
	 *
	 * @since   3.8.0
	 */
	function getRegistrationStates()
	{
		$list['2']  = Text::_('IC_PENDING');
		$list['1']  = Text::_('IC_COMPLETED');
		$list['0']  = Text::_('IC_CANCELLED');
		$list['-1'] = Text::_('IC_ERROR');
//		$list['-2'] = Text::_('IC_ABANDONED');
...
//			$list[$e->event] = $e->title . ' [' . $e->event . ']';
	...
	public function getBaseName()
	{
		if (!isset($this->basename))
		{
			...
			if (!empty($date))
			{
				...
				{
					$basename = str_replace('__DATE__', HTMLHelper::date($date, Text::_('DATE_FORMAT_LC3'), null)
	...
	protected function getEventTitle()
	{
		...
		{
			$title = Text::_('COM_ICAGENDA_NO_EVENT_TITLE');
		}
	...
	/**
	 * Get the Registration State name.
	 *
	 * @return  string  The Registration State name
	 *
	 * @since   3.8.13
	 */
	protected function getRegistrationStateName($state)
	{
		$states_array = [
			0 => 'IC_CANCELLED',
			1 => 'IC_COMPLETED',
		...
		foreach ($states_array as $key => $name) {
			if ($state == $key) {
				$registration_state_name = $name;
			}
		}

		return Text::_($registration_state_name);
	}
	...
	{
		$status_array = HTMLHelper::_('jgrid.publishedOptions');
	...
	public function getContent()
	{
		if (!isset($this->content)) {
			...
			foreach ($this->getItems() as $item) {
				...
				$customfields = icagendaCustomfields::getList($item->id, 1, 1, true);
			...
			if ($this->getState('event_title')) {
				$this->content .= str_replace('"', '""', Text::_('COM_ICAGENDA_REGISTRATION_EVENTID')) . '"';
			} else {
				$this->content .= '#' . '"';
			}

			if ($this->getState('registration_state')) {
				$this->content .= $separator . '"' . str_replace('"', '""', Text::_('COM_ICAGENDA_REGISTRATION_STATE')) . '"';
			}

			if ($this->getState('date')) {
				$this->content .= $separator . '"' . str_replace('"', '""', Text::_('COM_ICAGENDA_REGISTRATION_DATE')) . '"';
			}

			if ($this->getState('tickets')) {
				$this->content .= $separator . '"' . str_replace('"', '""', Text::_('COM_ICAGENDA_REGISTRATION_TICKETS')) . '"';
			}

			if ($this->getState('name')) {
				$this->content .= $separator . '"' . str_replace('"', '""', Text::_('IC_NAME')) . '"';
			}

			if ($this->getState('email')) {
				$this->content .= $separator . '"' . str_replace('"', '""', Text::_('COM_ICAGENDA_REGISTRATION_EMAIL')) . '"';
			}

			if ($this->getState('phone')) {
				$this->content .= $separator . '"' . str_replace('"', '""', Text::_('COM_ICAGENDA_REGISTRATION_PHONE')) . '"';
			}

			if ($this->getState('customfields')) {
				foreach ($header_cfs as $header) {
					$this->content .= $separator . '"' . str_replace('"', '""', Text::_($header)) . '"';
				}
			}

			if ($this->getState('notes')) {
				$this->content .= $separator . '"' . str_replace('"', '""', Text::_('COM_ICAGENDA_REGISTRATION_NOTES_DISPLAY_LABEL')) . '"';
			}

			if ($this->getState('status')) {
				$this->content .= $separator . '"' . str_replace('"', '""', Text::_('JSTATUS')) . '"';
			}

			if ($this->getState('created')) {
				$this->content .= $separator . '"' . str_replace('"', '""', Text::_('JGLOBAL_FIELD_CREATED_LABEL')) . '"';
			}

			if ($this->getState('reg_id')) {
				$this->content .= $separator . '"' . str_replace('"', '""', Text::_('JGLOBAL_FIELD_ID_LABEL')) . '"';
			}
			...
			foreach ($this->getItems() as $item) {
				...
				if ($this->getState('registration_state')) {
					$this->content .= $separator . '"' . str_replace('"', '""', $this->getRegistrationStateName($item->status)) . '"';
				}
				...
				if ($this->getState('date')) {
					...
					$this->content .= $separator . '"' .
						str_replace('"', '""', ($item->period == 1 ? Text::_('COM_ICAGENDA_REGISTRATION_ALL_DATES') : $regDate)) . '"';
				}
				...
				if ($this->getState('customfields')) {
					foreach ($values_cfs as $value) {
						$this->content .= $separator . '"' . str_replace('"', '""', Text::_($value)) . '"';
					}
				}
			...
			if ($this->getState('compressed')) {
				...
				// Run the packager
				$delete = Folder::files($app->get('tmp_path') . '/', uniqid('icagenda_registrations_'), false, true);

				if ( ! empty($delete)) {
					if (!File::delete($delete)) {
						// File::delete throws an error
						$this->setError(Text::_('COM_ICAGENDA_EXPORT_ERR_ZIP_DELETE_FAILURE'));
				...
				if ( ! $packager = $archive->getAdapter('zip')) {
					$this->setError(Text::_('COM_ICAGENDA_EXPORT_ERR_ZIP_ADAPTER_FAILURE'));

					return false;
				} elseif ( ! $packager->create($ziproot, $files)) {
					$this->setError(Text::_('COM_ICAGENDA_EXPORT_ERR_ZIP_CREATE_FAILURE'));
	...
	public function getItems()
	{
		if ($items = parent::getItems())
		{
			...
			{
				$result = Factory::getApplication()->triggerEvent('onICagendaRegistrationsGetItems', array('com_icagenda.registrations', &$item));
			}
		}
